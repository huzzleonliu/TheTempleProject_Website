...
${COMPOSE_CMD} -f "${REPO_ROOT}/docker-compose.yml" --profile dev up -d database >/dev/null

${COMPOSE_CMD} -f "${REPO_ROOT}/docker-compose.yml" --profile dev exec -T database bash -c '
set -euo pipefail
